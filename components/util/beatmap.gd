...
##
## Based on the provided [AudioStream], a respective .osu beatmap file, under
## the same name, will be searched for in the same directory and parsed. From the
...
class HitObjectInfo extends Resource:
...
class TimingPoint extends Resource:
...
@export var track : AudioStream :
	set(value):
		...
		self.path = '%s.osu' % value.resource_path.trim_suffix('.%s' % value.resource_path.get_file().get_extension())
...
static func load(beatmap_file_path : String, beatmap : Beatmap) -> void:
	if beatmap.is_initialized: return
	beatmap.objects.clear()
